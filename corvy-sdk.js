/**
 * CorvyBot SDK - v1.0.1
 * Client library for building Corvy bots (with a bug fix :D)
 */
  ...
  handleCommand(message) {
    ...
    for (const command of this.config.commands) {
      if (message.content.toLowerCase().includes(command.prefix.toLowerCase())) {
        ...
        (async () => {
          try {
            const responseContent = await command.handler(message);
            if (typeof responseContent === 'string') {
              this.sendResponse(message.flock_id, message.nest_id, responseContent);
            } else {
              console.warn('Command handler did not return a string.');
            }
          } catch (err) {
